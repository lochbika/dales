  ...
  subroutine initcrosssection
   ...
   implicit none
    ...
    if (lnetcdf) then
      ...
      call ncinfo(ncname1( 1,:),'uxz', 'xz crosssection of the West-East velocity','m/s','t0tt')
...
!> Do the xy crosssections and dump them to file
  ...
  subroutine wrtorth
  use modglobal, only : jmax,j1,kmax,nsv,rlv,cp,rv,rd,cu,cv,cexpnr,ifoutput,rtimee
  use modfields, only : um,vm,wm,thlm,qtm,svm,thl0,qt0,ql0,exnf,thvf,cloudnr
  use modmpi,    only : cmyid
  use modstat_nc, only : lnetcdf, writestat_nc
  implicit none
  ...
  allocate(thv0(2:j1,1:kmax),buoy(2:j1,1:kmax))


    do  j=2,j1
    do  k=1,kmax
      thv0(j,k) = (thl0(crossortho,j,k)+rlv*ql0(crossortho,j,k)/(cp*exnf(k))) &
                    *(1+(rv/rd-1)*qt0(crossortho,j,k)-rv/rd*ql0(crossortho,j,k))
