...
!      call ncinfo(ncname1( 5,:),'qtxz','xz crosssection of the Total water specific humidity','kg/kg','t0tt')
!      call ncinfo(ncname1( 6,:),'qlxz','xz crosssection of the Liquid water specific humidity','kg/kg','t0tt')
!      call ncinfo(ncname1( 7,:),'buoyxz','xz crosssection of the Buoyancy','K','t0tt')
!      call ncinfo(ncname1( 8,:),'qrxz','xz crosssection of the Rain water specific humidity','kg/kg','t0tt')
...
!    end if
    do cross=1,nxy
      ...
      call ncinfo(ncname2( 5,:),'qtxy','xy crosssections of the Total water specific humidity','kg/kg','tt0t')
      call ncinfo(ncname2( 6,:),'qlxy','xy crosssections of the Liquid water specific humidity','kg/kg','tt0t')
      call ncinfo(ncname2( 7,:),'buoyxy','xy crosssection of the Buoyancy','K','tt0t')
      call ncinfo(ncname2( 8,:),'qrxy','xy crosssection of the Rain water specific humidity','kg/kg','tt0t')
...
!    call ncinfo(ncname3( 5,:),'qtyz','yz crosssection of the Total water specific humidity','kg/kg','0ttt')
!    call ncinfo(ncname3( 6,:),'qlyz','yz crosssection of the Liquid water specific humidity','kg/kg','0ttt')
!    call ncinfo(ncname3( 7,:),'buoyyz','yz crosssection of the Buoyancy','K','0ttt')
!    call ncinfo(ncname3( 8,:),'qryz','yz crosssection of the Rain water specific humidity','kg/kg','0ttt')
...
!  use modfields, only : u0,v0,w0,thl0,qt0,sv0,thl0,ql0,exnf,thvf
...
!  real, allocatable :: vars(:,:,:),buoy(:,:)
!  real :: thv0

!  if( myid /= 0 ) return

!  allocate(buoy(2:i1,1:kmax))


!    do  i=2,i1
!    do  k=1,kmax
!      thv0 = (thl0(i,crossplane,k)+rlv*ql0(i,crossplane,k)/(cp*exnf(k))) &
!        *(1+(rv/rd-1)*qt0(i,crossplane,k)-rv/rd*ql0(i,crossplane,k))
!      buoy(i,k) = thv0-thvf(k)
!    enddo
!    enddo
...
!      vars(:,:,5) = qt0(2:i1,crossplane,1:kmax)
!      vars(:,:,6) = ql0(2:i1,crossplane,1:kmax)
!      vars(:,:,7) = buoy(2:i1,1:kmax)
!      if(nsv>1) then
!      vars(:,:,8) = sv0(2:i1,crossplane,1:kmax,iqr)
!      else
!      vars(:,:,8) = 0.
!      end if
...
!> Do the xy crosssections and dump them to file
  subroutine wrthorz
    ...
    if (lnetcdf) then
      ...
      vars(:,:,5) = qt0(2:i1,2:j1,crossheight(cross))
      vars(:,:,6) = ql0(2:i1,2:j1,crossheight(cross))
      vars(:,:,7) = buoy(2:i1,2:j1,cross)
      if(nsv>1) then
      vars(:,:,8) = sv0(2:i1,2:j1,crossheight(cross),iqr)
      else
      vars(:,:,8) = 0.
      end if
...
!    use modfields, only : u0,v0,w0,thl0,qt0,sv0,ql0,exnf,thvf
!    use modmpi,    only : cmyid
!    use modmicrodata, only : iqr
!    use modstat_nc, only : lnetcdf, writestat_nc
!    implicit none
...
!      vars(:,:,5) = qt0(crossortho,2:j1,1:kmax)
!      vars(:,:,6) = ql0(crossortho,2:j1,1:kmax)
!      vars(:,:,7) = buoy(2:j1,1:kmax)
!      if(nsv>1) then
!      vars(:,:,8) = sv0(crossortho,2:j1,1:kmax,iqr)
!      else
!      vars(:,:,8) = 0.
!      end if
