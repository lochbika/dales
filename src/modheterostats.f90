...
save
  ...
  integer :: OHISOcovid, O3NOcovid           !HGO: OH-Isoprene & O3-NO covariances
...
!    write(6,*) "HETEROSTATS: Creating: ", ncfile
    ...
    status = nf90_def_var(ncid, "OHISOcov", nf90_float, (/yid, zid, tid/), OHISOcovid) !HGO: OH-Isoprene & O3-NO covariances
    if (status /= nf90_noerr) call nchandle_error(status)                              !HGO: OH-Isoprene & O3-NO covariances
    status = nf90_def_var(ncid, "O3NOcov", nf90_float, (/yid, zid, tid/), O3NOcovid)   !HGO: OH-Isoprene & O3-NO covariances
    if (status /= nf90_noerr) call nchandle_error(status)                              !HGO: OH-Isoprene & O3-NO covariances
  ...
  subroutine do_heterostats
    ...
    real, dimension(jmax,ncklimit)     :: OHISOcov, O3NOcov                   !HGO: OH-Isoprene & O3-NO covariances
    ...
    OHISOcov(:,:) = 0.0                 !HGO: OH-Isoprene & O3-NO covariances
    O3NOcov       = 0.0                 !HGO: OH-Isoprene & O3-NO covariances
      ...
      do j = 1,jmax
        do i = 1,imax
          ...
          if(k==1) then
            ...
            wqlcovs(j,k) = wqlcovs(j,k) - 0.5*(ekh(i+1,j+1,k)+ekh(i+1,j+1,k-1)) * (ql0(i+1,j+1,k) - ql0(i+1,j+1,k-1)) / dz
          endif
        end do
      end do
    end do
    ...
    do k = 1,ncklimit            !HGO: OH-Isoprene & O3-NO covariances
      do j = 1,jmax              !HGO: OH-Isoprene & O3-NO covariances
        do i = 1,imax            !HGO: OH-Isoprene & O3-NO covariances
          !shift prognostic fields one step as 1st column            !HGO: OH-Isoprene & O3-NO covariances
          !is dummy column because of MPI and periodicity            !HGO: OH-Isoprene & O3-NO covariances
          OHISOcov(j,k) = OHISOcov(j,k) + (sv0(i+1,j+1,k,10)-svavg(j,k,10)) * (sv0(i+1,j+1,k,12)-svavg(j,k,12)) !ISO = 10, OH = 12            !HGO: OH-Isoprene & O3-NO covariances
        end do            !HGO: OH-Isoprene & O3-NO covariances
      end do              !HGO: OH-Isoprene & O3-NO covariances
    end do                !HGO: OH-Isoprene & O3-NO covariances
    ...
    do k = 1,ncklimit            !HGO: OH-Isoprene & O3-NO covariances
      do j = 1,jmax              !HGO: OH-Isoprene & O3-NO covariances
        do i = 1,imax            !HGO: OH-Isoprene & O3-NO covariances
          !shift prognostic fields one step as 1st column            !HGO: OH-Isoprene & O3-NO covariances
          !is dummy column because of MPI and periodicity            !HGO: OH-Isoprene & O3-NO covariances
          O3NOcov(j,k) = O3NOcov(j,k) + (sv0(i+1,j+1,k,2)-svavg(j,k,2)) * (sv0(i+1,j+1,k,4)-svavg(j,k,4)) !O3 = 2, NO = 4            !HGO: OH-Isoprene & O3-NO covariances
        end do            !HGO: OH-Isoprene & O3-NO covariances
      end do              !HGO: OH-Isoprene & O3-NO covariances
    end do                !HGO: OH-Isoprene & O3-NO covariances
    ...
    OHISOcov = OHISOcov / imax            !HGO: OH-Isoprene & O3-NO covariances
    O3NOcov  = O3NOcov  / imax            !HGO: OH-Isoprene & O3-NO covariances
    ...
    status = nf90_put_var(ncid, OHISOcovid, OHISOcov, (/1,1,nccall/), (/jmax, ncklimit, 1/)) !HGO: OH-Isoprene & O3-NO covariances
    if(status /= nf90_noerr) call nchandle_error(status)                                     !HGO: OH-Isoprene & O3-NO covariances
    status = nf90_put_var(ncid, O3NOcovid, O3NOcov, (/1,1,nccall/), (/jmax, ncklimit, 1/)) !HGO: OH-Isoprene & O3-NO covariances
    if(status /= nf90_noerr) call nchandle_error(status)                                     !HGO: OH-Isoprene & O3-NO covariances
